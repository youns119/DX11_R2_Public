...
HRESULT CMaterial::Init(HANDLE& _hFile, _ulong& _dwByte, const _tchar* _szModelName)
{
	for (size_t i = 0; i < MATERIAL_MAX; i++)
	{
		...
		for (size_t j = 0; j < iNumTexture; j++)
		{
			...
			strFinalPath = MODEL_TEXTURE + strFinalPath + L"/" + szFinalPath;
...
{
	if (_iTextureIndex >= m_vecMtrlTexture[_eMaterial].size() ||
		m_vecMtrlTexture[_eMaterial][_iTextureIndex] == nullptr)
		return S_OK;

	return _pShader->Bind_SRV(_pConstantName, m_vecMtrlTexture[_eMaterial][_iTextureIndex]);
}
