...
CVIBuffer_Instancing_Point::CVIBuffer_Instancing_Point(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext)
	: CVIBuffer_Instancing{ _pDevice, _pContext }
{
}

CVIBuffer_Instancing_Point::CVIBuffer_Instancing_Point(const CVIBuffer_Instancing_Point& _Prototype)
	: CVIBuffer_Instancing(_Prototype)
{
}

HRESULT CVIBuffer_Instancing_Point::Init(const FVIBuffer_Instancing* _pDesc)
{
	const FVIBuffer_Instancing_Point* pDesc = static_cast<const FVIBuffer_Instancing_Point*>(_pDesc);
...
#pragma region VERTEX_BUFFER
	...
	VTXPOINT* pVertices = new VTXPOINT[m_iNumVertice];
	ZeroMemory(pVertices, sizeof(VTXPOINT) * m_iNumVertice);
...
HRESULT CVIBuffer_Instancing_Point::Init(void* _pDesc)
{
...
CVIBuffer_Instancing_Point* CVIBuffer_Instancing_Point::Create
(
...
{
	CVIBuffer_Instancing_Point* pInstance = new CVIBuffer_Instancing_Point(_pDevice, _pContext);

	if (FAILED(pInstance->Init(_pDesc)))
	{
		MSG_BOX("Failed To Created : CVIBuffer_Instancing_Point");
...
CClonable* CVIBuffer_Instancing_Point::Clone(void* _pDesc)
{
